...
def train_experiment(quick: bool = False):
    ...
    print(f"\n{'='*60}")
    print("OOD Evaluation (Proper Metrics)")
    print(f"{'='*60}")
    ...
    for test_size in [16, 24, 32]:
        ...
        ood_results[f"grid_{test_size}x{test_size}"] = {
            'model': metrics,
            'compiled_solver': solver_metrics,
        }
        
        print(f"Grid {test_size}x{test_size}:")
        print(f"  Model:    exact={metrics['exact_match']:.3f}, fg_acc={metrics['foreground_acc']:.3f}, obj_iou={metrics['object_iou']:.3f}")
        print(f"  Compiled: exact={solver_metrics['exact_match']:.3f}, fg_acc={solver_metrics['foreground_acc']:.3f}, obj_iou={solver_metrics['object_iou']:.3f}")
    ...
    for n_obj_range in [(1, 4), (5, 7), (8, 10)]:
        ...
        print(f"Objects {n_obj_range[0]}-{n_obj_range[1]}:")
        print(f"  Model:    exact={metrics['exact_match']:.3f}, fg_acc={metrics['foreground_acc']:.3f}, obj_iou={metrics['object_iou']:.3f}")
        print(f"  Compiled: exact={solver_metrics['exact_match']:.3f}, fg_acc={solver_metrics['foreground_acc']:.3f}, obj_iou={solver_metrics['object_iou']:.3f}")
    ...
    for dx, dy in shifts:
        ...
        print(f"  Shift ({dx:+d}, {dy:+d}): {match_rate:.4f}")
    
    commutation_mean = np.mean(commutation_scores)
    print(f"  Mean commutation: {commutation_mean:.4f}")
